...
TH1D *h2, *h22;
TH1D *h3 =new TH1D("hrl", "response vs sensor(6x6 cm^{2}) right&left edge position, Run3 TT; |x_{min}| coordinate [cm]; Sensor response, kHz", 70, 0, 70);
TH1D *h33 =new TH1D("left", "response vs sensor(6x6 cm^{2}) right&left edge position, Run3 TT; |x_{min}| coordinate [cm]; Sensor response, kHz", 70, 0, 70);
TH1D *h8, *h88;
TH1D *h9 =new TH1D("htl", "response vs sensor(6x6 cm^{2}) bottom&top edge position, Run3 TT; |y_{min}| coordinate [cm]; Sensor response, kHz", 70, 0, 70);
TH1D *h99 =new TH1D("bot", "response vs sensor(6x6 cm^{2}) bottom&top edge position, Run3 TT; |y_{min}| coordinate [cm]; Sensor response, kHz", 70, 0, 70);
...
h3->SetBinContent(i+1,(h2->GetEntries())*4.1/1000);
h33->SetBinContent(i+1,h22->GetEntries()*4.1/1000);
}
c1->cd();
...
//cout<<"i="<<i<<", NoH="<<h4->GetEntries()<<"\n";
h9->SetBinContent(i+1,(h8->GetEntries())*4.1/1000);
h99->SetBinContent(i+1,(h88->GetEntries())*4.1/1000);
}
c4->cd();
